...
function createRecordDistrictUrl(regionName, districtName) {
  ...
  regionGrid.innerHTML = filteredRegions.length ? filteredRegions.map((region) => `
    <article class="region-card">
      ...
      <div class="district-list">
        ${region.districts.map((district) => `<a class="district-link" href="${createRecordDistrictUrl(region.name, district)}">${district} 내신 과외</a>`).join('')}
      </div>
    </article>
  `).join('') : '<p>검색한 지역의 내신 대비 과외 안내가 준비 중입니다. 무료 상담으로 문의해주세요.</p>';
}
