...
if (!region || !region.districts.includes(districtName) || (neighborhoodName && !neighborhoods.includes(neighborhoodName))) {
  ...
  detailContent.innerHTML = `
    ...
    ${neighborhoodName ? `<section class="city-section neighborhood-keyword-section"><div class="city-container"><div class="city-section-heading"><div><p class="city-kicker">NEARBY GED SUPPORT</p><h2>${areaLabel} 아파트·인근 검정고시 안내</h2></div><p>실제 상담 지역과 가까운 생활권을 기준으로 안내합니다.</p></div>${apartments.length ? `<div class="apartment-list">${apartments.map((apartment) => `<span class="apartment-tag">${apartment}</span>`).join('')}</div>` : `<p class="keyword-empty">등록된 아파트 정보가 준비 중입니다. 아파트명을 추가하면 인근 키워드가 함께 생성됩니다.</p>`}<div class="keyword-list">${neighborhoodKeywords.map((keyword) => `<span class="keyword-tag">${keyword}</span>`).join('')}</div></div></section>` : ''}
