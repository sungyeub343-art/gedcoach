...
function renderNotFound() {
  document.title = '지역 안내를 찾을 수 없습니다 | 상상코칭 검정고시';
...
if (!region || !region.districts.includes(districtName) || (neighborhoodName && !neighborhoods.includes(neighborhoodName))) {
  ...
  const showSajikGuide = regionName === '서울특별시' && districtName === '종로구' && neighborhoodName === '사직동';
  ...
  document.title = `${title} 검정고시 과외 | 상상코칭 검정고시`;
  ...
  detailContent.innerHTML = `
    ...
    ${showSajikGuide ? `<section class="city-section city-guide-section"><div class="city-container"><img class="city-guide-image" src="../[복사본] 검정고시_이미지.jpg" alt="검정고시 대상, 과목, 일정, 시험 시간표, 수업 방식과 후기를 안내하는 이미지" loading="lazy" /></div></section>` : ''}
    ${neighborhoodName ? `<section class="city-section neighborhood-keyword-section"><div class="city-container"><div class="city-section-heading"><div><p class="city-kicker">NEARBY GED SUPPORT</p><h2>${nearbyHeading}</h2></div><p>실제 상담 지역과 가까운 생활권을 기준으로 안내합니다.</p></div>${apartments.length ? `<h3 class="keyword-heading">아파트·주요 생활권</h3><div class="apartment-list">${apartments.map((apartment) => `<span class="apartment-tag">${apartment}</span>`).join('')}</div>` : ''}${learningKeywords.length ? `<h3 class="keyword-heading">학원가·교육 키워드</h3><div class="keyword-list">${learningKeywords.map((keyword) => `<a class="keyword-tag" href="${createKakaoMapSearchUrl(keyword)}" target="_blank" rel="noopener">${keyword}</a>`).join('')}</div>` : ''}<h3 class="keyword-heading">검정고시 상담 키워드</h3><div class="keyword-list">${neighborhoodKeywords.map((keyword) => `<span class="keyword-tag">${keyword}</span>`).join('')}</div></div></section>` : ''}
